...
function setupPopup() {
  const voiceSelectionBrowser = document.getElementById('voice-selection-browser');
  ...
  // Restore the useApi setting when the popup is opened
  chrome.storage.local.get('useApi', (items) => {
    useApiCheckbox.checked = items.useApi;
  });

  // Load the saved API URL from storage and set the input value
  ...
  submitUrlButton.addEventListener('click', () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.sendMessage(tabs[0].id, { type: "setApiUrl", url: url });
    });
  });

  // Save the useApi setting when the checkbox is clicked
  useApiCheckbox.addEventListener('click', () => {
    const useApi = useApiCheckbox.checked;
    chrome.storage.local.set({ useApi }, () => {
      console.log('Use API setting saved:', useApi);
    });
  });
  ...
  if (typeof speechSynthesis !== 'undefined') {
    if (speechSynthesis.onvoiceschanged !== undefined) {
      speechSynthesis.onvoiceschanged = () => {
        populateVoiceList(voiceSelectionBrowser);
      };
    }
  }

  // Load the currently selected browser TTS voice from storage
  chrome.storage.local.get(['selectedVoiceBrowser'], (result) => {
    if (result.selectedVoiceBrowser) {
      voiceSelectionBrowser.value = result.selectedVoiceBrowser;
    }
  });

  // Save the selected browser TTS voice to storage
  voiceSelectionBrowser.addEventListener('change', () => {
    chrome.storage.local.set({ selectedVoiceBrowser: voiceSelectionBrowser.value });
  });
